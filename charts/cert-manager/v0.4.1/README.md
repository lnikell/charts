...
| `ingressShim.defaultIssuerName` | Optional default issuer to use for ingress resources | `letsencrypt-prod` |
| `ingressShim.defaultIssuerKind` | Optional default issuer kind to use for ingress resources | `ClusterIssuer` |
| `ingressShim.defaultACMEChallengeType` | Optional default challenge type to use for ingresses using ACME issuers | `http01` |
...
|`letsencrypt.server`| Let's Encrypt Server/Environment | https://acme-v02.api.letsencrypt.org/directory |
|`letsencrypt.email`| Let's Encrypt Notification Email | |
